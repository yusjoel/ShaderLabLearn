Shader "3D Graphics with XNA Games Studio 4.0/Chapter 5/01.Fog"
{
	Properties
	{
		...
		_FogStart("Fog Start", Float) = 2000
	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 PixelShaderFunction(VertexShaderOutput input) : SV_Target
			{
				...
				float fog = clamp((dist - _FogStart) / (_FogEnd - _FogStart), 0, 1);
				return lerp(output, _FogColor, fog);
			}
			ENDCG
		}
	}
