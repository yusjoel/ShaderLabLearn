Shader "CookbookShaders/Chapter005/CarPaint"
{
    Properties
    {
        _Color ("Color", Color) = (1, 1, 1, 1)
        _MainTex ("Albedo (RGB)", 2D) = "white" { }
        _SpecColor ("Specular Color", Color) = (1, 1, 1, 1)
        _SpecPower ("Specular Power", Range(0.01, 30)) = 3
        _ReflCube ("Reflection Cube", CUBE) = "" { }
        _BrdfTex ("BRDF Texture", 2D) = "" { }
        _DiffPower ("Diffuse Power", Range(0.01, 10)) = 0.5
        _FalloffPower ("Falloff Spread", Range(0.01, 10)) = 3
        _ReflAmount ("Reflection Amount", Range(0.01, 1)) = 0.5
        _ReflPower ("Reflection Power", Range(0.01, 3)) = 2
    }
    
    SubShader
    {
        Tags { "RenderType" = "Opaque" }
        LOD 200
        
        CGPROGRAM
        
        #pragma surface surf CarPaint
        #pragma target 3.0

        fixed4 _Color;
        sampler2D _MainTex;
        fixed _SpecPower;
        samplerCUBE _ReflCube;
        sampler2D _BrdfTex;
        fixed _DiffPower;
        fixed _FalloffPower;
        fixed _ReflAmount;
        fixed _ReflPower;

        struct Input
        {
            float2 uv_MainTex;
            float3 worldRefl;
            float3 viewDir;
        };

        void surf(Input IN, inout SurfaceOutput o)
        {
            // Albedo comes from a texture tinted by color
            fixed4 c = tex2D(_MainTex, IN.uv_MainTex) * _Color;
            o.Albedo = c.rgb;
            o.Alpha = c.a;

            fixed falloff = saturate(1 - dot(normalize(IN.viewDir), o.Normal));
            falloff = pow(falloff, _FalloffPower);
            float3 refl = texCUBE(_ReflCube, IN.worldRefl).rgb;
            float3 emission = pow(refl * falloff, _ReflPower) * _ReflAmount;
            o.Emission = emission;

            o.Specular = c.r;
            o.Gloss = 1;
        }

        inline fixed4 LightingCarPaint(SurfaceOutput s, fixed3 lightDir, half3 viewDir, fixed atten)
        {
            half3 h = normalize(lightDir + viewDir);
            fixed diff = max(0, dot(s.Normal, lightDir));

            float ahdn = 1 - dot(h, normalize(s.Normal));
            ahdn = pow(clamp(ahdn, 0, 1), _DiffPower);
            half4 brdf = tex2D(_BrdfTex, float2(diff, 1 - ahdn));

            float nh = max(0, dot(s.Normal, h));
            float spec = pow(nh, s.Specular * _SpecPower) * s.Gloss;

            fixed4 c;
            fixed3 diffColor = s.Albedo * _LightColor0.rgb * brdf.rgb;
            fixed3 specColor = _LightColor0.rgb * _SpecColor.rgb * spec;
            c.rgb = (diffColor + specColor) * atten;
            c.a = s.Alpha + _LightColor0.a * _SpecColor.a * spec * atten;

            return c;
        }
        
        ENDCG
        
    }
    FallBack "Diffuse"
}
